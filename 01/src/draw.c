...
unsigned char convertValue(double value) {
   // unsigned char res = (unsigned char)(value * 4 * 255);
   // printf("Value:%f  Converted value:%d\n", value, res);
    return (unsigned char)(value * 4 * 255);
}
...
void draw_grid(double* grid, uint32_t x, uint32_t y, const char* filepath) {
   ...
   // char* data = { "P3 128 128 256 "};

//debugger srand(time(NULL));
