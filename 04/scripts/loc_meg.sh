...
for size_KiB in "${dist[@]}"; do
	#likwid-perfctr -O --stats -C 0 -g FLOPS_DP -c 0 -m -e FLOPS_DP ../task_4 "$size_KiB" >> result_likwid_local.csv
	#likwid-perfctr -O --stats -C 0 -g DATA -c 0 -m ../task_4 "$size_KiB" >> result_likwid_local.csv
	likwid-perfctr -C 0 -g DATA ../task_4 "$size_KiB" >> result_likwid_local.csv
